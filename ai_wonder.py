...
from input_handler import input_piped_model
...
def inverse_transform(state, dataset, nums_only=False):
    ...
    if len(state.num_cols) > 0:
        num_cols = pd.DataFrame(
            np.round(state.num_scaler.inverse_transform(dataset[state.num_cols]), 4),
            columns=state.num_cols
        )
...
def align_features(state, data):
    """
    Align features in the internal order (cat_cols + num_cols)
    """
    return inverse_transform(state, state.transformers.transform(data))
...
# Counterfactuals
def what_if_instances(state, point, test_data):
    # Prep data
    point = state.transformers.transform(point)
    test_data = state.transformers.transform(test_data)
    ...
    # Order them in the increasing order of distances
    ...
    pred_cfs = fix_target_type(state.model.predict(total_cfs.values), state.y[state.target].dtype)
    final_cfs = inverse_transform(state, total_cfs)
    final_cfs[f"{state.target}_"] = pred_cfs
    # final_cfs['distance'] = np.round(distances.flatten()[nearest_indexes], 4)[0]
    return final_cfs
